...
import citlab.importer.featureide.FeatureIdeModelImporterTest;
import citlab.importer.featureide.SxfmFiImporter;
...
//	}
	...
	public void testAllmodifiedModels(){
		List<String> testModelsIn = testModelsIn("splotmodels_new");
		if (testModelsIn.isEmpty()) return;
		System.err.println(testModelsIn);
		fail("some models are not imported");
	}
	...
	private List<String> testModelsIn(String dirPath){
		...
		for (int i = 0; i < children.length; i++) {
			...
			try {
				...
				File resultFile = new File("modelCITL"+File.separator+model.getName()+".citl");
				if(resultFile.exists())
					 resultFile = new File("modelCITL"+File.separator+model.getName()+"1"+".citl");
