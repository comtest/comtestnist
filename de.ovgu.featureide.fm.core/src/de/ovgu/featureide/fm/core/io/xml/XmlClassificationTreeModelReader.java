...
import de.ovgu.featureide.fm.core.RootFeature;
import de.ovgu.featureide.fm.core.io.AbstractFeatureModelReader;
import de.ovgu.featureide.fm.core.io.UnsupportedModelException;

/**
 * Parses a Classification Tree Model from XML
 * 
 * @author Wayman Tan
 */
public class XmlClassificationTreeModelReader extends AbstractFeatureModelReader implements XMLFeatureModelTags {

	public XmlClassificationTreeModelReader(FeatureModel featureModel) {
		...
		for (Element e : getElements(nodeList)) {
			...
			if (e.hasAttributes()) {
				...
				for (int i = 0; i < nodeMap.getLength(); i++) {
					...
					} else if (attributeName.equals(MINIMUM)) {
						isMin = attributeValue.equals(TRUE);
			...
			if (nodeName.equals(ROOT)) {
				...
				f = new ClassificationFeature(featureModel, name);
			} else if (nodeName.equals(CLASS)) {
				f = new ClassFeature(featureModel, name);
			} else if (nodeName.equals(RANGECLASS)) {
				f = new RangeClassFeature(featureModel, name);
			} else {
				throwError("Unknown feature type: " + nodeName, e);
			}
			
			if (f instanceof ClassificationFeature) {
				if (!dataType.isEmpty()) {
					((ClassificationFeature)f).setDataType(dataType);
				}
			}
			
			if (f instanceof ClassFeature) {
				if (!value.isEmpty()) {
					((ClassFeature)f).setValue(value);
				}
			}
			
			if (f instanceof RangeClassFeature) {
				((RangeClassFeature)f).setMin(isMin);				
			}
		...
		for (Element e: getElements(nodeList)) {
			for (Element child: getElements(e.getChildNodes())) {
				...
				if (nodeName.equals(RULE)) {
					//Douglas
					Constraint c = new Constraint(featureModel, parseConstraints2(child.getChildNodes()).getFirst().toString(), parseConstraints2(child.getChildNodes()).getFirst());
