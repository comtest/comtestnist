...
import citlab.core.ui.utils.CombCounterJob;
...
public class OutputDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		TableColumn[] columnOfTheModel = new TableColumn[inputlist.getTests().get(0).getAssignments()
				.size()+1];

		columnOfTheModel[0] = new TableColumn(table, SWT.NONE);
		columnOfTheModel[0].setText("Test");
		int n = 1;
		for (Assignment i :inputlist.getTests().get(0).getAssignments()) {
		...
		btnNewButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
			    CombCounterJob cjob = new CombCounterJob("comb", inputlist.getModel());
				cjob.setUser(true);
				cjob.schedule();
