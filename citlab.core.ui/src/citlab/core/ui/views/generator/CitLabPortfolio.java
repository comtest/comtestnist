...
import citlab.core.ui.utils.Statistics;
...
public class CitLabPortfolio {
	...
	private boolean ignoreTestgoals;
	private boolean ignoreConstraints;
	private boolean ignoreSeeds;
	private int nWise;
	...
	public CitLabPortfolio(boolean useSimplifier, boolean ignoreTestgoals,
			boolean ignoreConstraints, boolean ignoreSeeds, int nwise) {
		this.useSimplifier = useSimplifier;
...
//	}
	...
	private void generate(
			...
			Map<String, Pair<Double, Double>> dataSizeTime)
		...
		// generate
		for (Pair<ICitLabTestGenerator, CitModel> pair : generators) {
...
//			else
				...
				for (int i = 0; i < nt; i++) {
					System.out.println("**** model "
							+ pair.getSecond().getName() + " run: " + i
							+ " generator: " + pair.getSecond());
					try {

						Job generation = new SafeGeneratorRunnable(generator,
								citModel);
			...
			if (!error) {
				...
				// sizesS.getStdDev()));
				dataRun.put(pair.getSecond().getName(),
						new Pair<Double, Double>(meanSize, meanTime));
...
//			printOutput.flush();
	...
	//	private PrintStream printOutput;
		...
		public SafeGeneratorRunnable(ICitLabTestGenerator generator,
				CitModel citmodel) {
		...
		protected IStatus run(final IProgressMonitor monitor) {
				...
				public void run() {
					...
					if (useSimplifier) {
						...
						output = generator.generateTestsAndInfo(
								simp.getSimplifiedVersion(), ignoreConstraints,
								ignoreSeeds, ignoreTestgoals, nWise);
						SemanticPreserver SP = new SemanticPreserver(output,
								simp);
						output = SP.preserve();
					} else {
			...
			IsStoppedPolling stopper = new IsStoppedPolling("", monitor, false,
					generator);
						...
						try {
							...
							TestSuiteView view = (TestSuiteView) PlatformUI
									...
									.showView(
											...
											"citlab.core.ui.outputSecondaryID"
													+ output.getModel()
															.getName()
													+ "  "
