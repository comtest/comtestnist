...
import citlab.core.util.IsStoppedPolling;
...
import citlab.testsuite.util.SemanticPreserver;
		...
		protected IStatus run(final IProgressMonitor monitor) {
			...
			ISafeRunnable runnable = new ISafeRunnable() {
				...
				public void run()  {
						...
						if (useSimplifier) {
							Simplificator simp = new Simplificator(citModel);
							output = generator.generateTestsAndInfo(
									simp.getSimplifiedVersion(),
									ignoreConstraints, ignoreSeeds,
									ignoreTestgoals, nWise);
							SemanticPreserver SP = new SemanticPreserver(
									output, simp);
							output = SP.preserve();
						} else {
							output = generator.generateTestsAndInfo(citModel,
									ignoreConstraints, ignoreSeeds,
									ignoreTestgoals, nWise);
						}
				...
				public void run() {
					...
					if (output != null) {
						try {
							...
							TestSuiteView view = (TestSuiteView) PlatformUI
									.getWorkbench()
									.getActiveWorkbenchWindow()
									.getActivePage()
									.showView(
											"citlab.core.ui.output",
											"citlab.core.ui.outputSecondaryID"+output.getModel().getName()+"  "
													+ output.getGeneratorName()+"   "+dateFormat.format(date).replace(":", "__"),IWorkbenchPage.VIEW_ACTIVATE
											);
...
//						outD.open();
					...
					throw new RuntimeException(
							"Output null problem with citModel parsing");}
					}
	...
	private ICitLabTestGenerator generator;
	...
	@Inject
	Shell shell;
	public GeneratorDialog(Shell parent, int style, CitModel citModel,
		...
		supportConstraints = (attribute != null)
	...
	private void createContents() {
		shlGeneratorDialog = new Shell(getParent(), SWT.DIALOG_TRIM | SWT.MIN
				| SWT.MAX | SWT.APPLICATION_MODAL);
		shlGeneratorDialog.setSize(588, 282);
		...
		Composite composite_1 = new Composite(shlGeneratorDialog, SWT.BORDER
		...
		lblAlgorithmLabel.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_WIDGET_LIGHT_SHADOW));
		lblAlgorithmLabel.setBounds(5, 10, 281, 26);
		lblAlgorithmLabel.setAlignment(SWT.CENTER);
		lblAlgorithmLabel.setFont(SWTResourceManager.getFont(
				"Courier 10 Pitch", 18, SWT.BOLD));
		lblAlgorithmLabel.setText(generatorName);

		Button btnGenerate = new Button(composite, SWT.NONE);
		btnGenerate.setBounds(187, 191, 76, 26);
		...
		final Button btnIgnoreConstraints = new Button(composite_uses,
				SWT.CHECK);
		...
		spinner.setLocation(133, 1);
		spinner.setSize(53, 28);
		spinner.setForeground(SWTResourceManager.getColor(SWT.COLOR_DARK_GRAY));
		...
		lblNewLabel.setSize(100, 29);
		lblNewLabel.setFont(SWTResourceManager.getFont("Saab", 16, SWT.NORMAL));
		...
		lblNewLabel.setText("N-WISE");
				...
				btnSetProperties.setBounds(54, 191, 127, 26);
				btnSetProperties.setEnabled(supportProperties);
						...
						btnSetProperties.addSelectionListener(new SelectionAdapter() {
							...
							public void widgetSelected(SelectionEvent e) {
								...
								GenPropertiesDialog genD = new GenPropertiesDialog(shell,
									generator.getGenProperties());
								genD.open();
							}
						});
	...
	private void setGenerator(IConfigurationElement element) {
		try {
			...
			if (o instanceof ICitLabTestGenerator) {
				generator = (ICitLabTestGenerator) o;
			} else {
